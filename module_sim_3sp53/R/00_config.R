...
PATHS <- list(
  module = path.expand("~/gitlab/LDscnR-paper/module_sim_3sp53"),
  out    = path.expand("~/gitlab/LDscnR-paper/module_sim_3sp53/out"),
  ...
  ## [!] REPOINTED 2026-09-08 (PK: new production sims replace bgs5 as the
  ## primary comparison, but bgs5 itself is KEPT -- archived permanently at
  ## ../module_sim_bgs5/, not superseded). This is a NEW, SEPARATE module
  ## (module_sim_3sp53, not a repoint of the live module_sim/) so that
  ## module_sim/ itself keeps working unmodified on either machine, and so
  ## this module's own paths can be mini-local without breaking anything else.
  ## Source: /Volumes/Large_storage/prod_out on Petri's mini
  ## (petrikemppainen@192.168.1.150), verified complete by the NEMO session
  ## (1400 adapt runs + 140 burn-ins, 125 GB, 0 failures, every expected
  ## output present and non-empty, no error/abort/segfault in any log) --
  ## NOT re-verified independently here, taken on that report. Design: 7
  ## cells x 10 reps x 10 envs x 2 arms = 1400, ALL 7 cells present this time
  ## (unlike bgs5's 4/7) -- V1_c2/V2_c2 deliberately excluded, same as
  ## before. Finer map resolution than bgs5 (MAP_RES 3.3e-6 vs 3.3e-5) --
  ## the NEMO session's own measurement: this roughly halves the apparent
  ## BGS effect relative to bgs5 (the old maps were coarse enough that
  ## 32-44% of loci sat at zero recombination distance from a neighbour; the
  ## new maps have 1.5-1.8%) -- expected, not a regression, and not
  ## comparable number-for-number against bgs5's BGS-effect figures without
  ## accounting for it.
  raw_nemo_nobgs = "/Volumes/Large_storage/prod_out",
  raw_nemo_bgs5  = "/Volumes/Large_storage/prod_out",   # kept named _bgs5 for structural parity with module_sim/00_config.R; both tags live in the one directory here too
  ## Reference chromosome maps + env surfaces -- same role as raw_recmap_dir/
  ## raw_env_dir always had, new location: ~/LDscnR-NEMO/params_3spC_53cM on
  ## the mini (10 rec_map*.rds under its rds/ subdir, env_1..10.txt at top
  ## level -- verified present and correctly named, 2026-09-08).
  raw_recmap_dir = path.expand("~/LDscnR-NEMO/params_3spC_53cM/rds"),
  raw_env_dir    = path.expand("~/LDscnR-NEMO/params_3spC_53cM"),
  ## R_parsing/'s output -- same role as module_sim/00_config.R's `parsed`,
  ## a fresh directory name (not reusing module_sim's own) so a stray file
  ## from the wrong raw source can never be silently read.
  parsed = "/Volumes/Large_storage/module_sim_3sp53_parsed",
  cache  = file.path(path.expand("~/gitlab/LDscnR-paper/module_sim_3sp53"), "cache")
)
...
## REPLICATES. [!] 2026-09-08: ALL 7 cells present this time (unlike bgs5's
## partial 4/7) -- the NEMO session verified all 1400 archives complete, 0
## missing. Grid drivers' existence-check-and-skip logic is harmless here
## (nothing will ever be skipped) and is left as-is rather than removed, so
## this module stays structurally identical to module_sim/'s drivers.
##
...
CELLS <- "V2_c1"          # widen to the full 7-cell CELLS_ALL once verified
