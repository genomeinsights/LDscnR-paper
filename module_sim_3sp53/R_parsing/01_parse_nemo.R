...
## [!] 2026-09-08, module_sim_3sp53: unlike module_sim's raw .tgz archives
## (one per combo, needing untar() into a scratch directory), the new
## production data at /Volumes/Large_storage/prod_out on the mini is
## ALREADY UNPACKED -- one directory per (tag,chr,cell,env), holding
## GENO/<run>_1000_1.{map,snp_geno}, stats/<run>.txt, <run>.log (NEMO
## session's report, spot-checked directly 2026-09-08: exactly one .map and
## one .snp_geno per run's GENO/, no cross-env contamination -- unlike the
## old .tgz format, each run already lives in its own directory, so the
## "some archives bundle >1 env together" problem module_sim/'s version
## guarded against is structurally impossible here). No untar step, no
## scratch directory, no PATHS$untar -- read straight from the raw location.
...
## ---- 1. locate the already-unpacked run's GENO files ---------------------------
## Resolved BEFORE the INPUTS/stage_stale block below (unlike module_sim/'s
## version, which only had a single archive FILE to hash) because
## write_receipt()/stage_stale() hash each INPUTS entry as a file (sha() on
## a directory errors) -- INPUTS below therefore needs the two actual files,
## not geno_dir itself.
...
INPUTS <- c(map_file = map_file, geno_file = geno_file, recmap = recmap_rds, env = env_txt)
...
## [!] no unlink here (module_sim/'s version cleaned up its untar scratch
## copy at this point) -- geno_dir IS the permanent raw data, never a copy.
...
saveRDS(list(GTs = GTs, map = map, env = env,
            source = list(geno_dir = geno_dir, recmap = recmap_rds, env_file = env_txt)), OUT)
