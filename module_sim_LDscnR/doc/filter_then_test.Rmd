...
## What the representative costs, measured

Two measurements on one pooled panel (V0.5_c1 nobgs env3, 294,609 SNPs in 99,787
stage-2 clusters), comparing the same EMMAX p-values at both resolutions.

```{r reptab, results="asis"}
cat("\\begin{tabular}{@{}lrrrr@{}}\n\\toprule\n")
cat("Tested & tests & BH cutoff $p$ & significant & driving QTN passing \\\\\n\\midrule\n")
cat("every SNP & 294,609 & 4.2e-04 & 2,467 & 14 of 21 \\\\\n")
cat("LD-central representatives & 99,787 & 1.2e-05 & 27 & --- \\\\\n")
cat("\\bottomrule\n\\end{tabular}\n")
```

\begin{keybox}
\textbf{Fewer tests made the threshold harsher, not easier.} Three times fewer
tests and a BH cutoff \textbf{35 times more severe}. BH is adaptive --- its
cutoff is the largest $p$ with $p \le \alpha i/n$, so it depends on how many
small p-values survive, not on $n$ alone. Restricting to representatives removed
most of the signal along with most of the tests, and the step-up never climbs.

This qualifies the rationale for filtering stated in Section~\ref{sec:fdr}:
a lighter multiplicity correction follows only if the filter \emph{retains} the
signal. Where it does not, reducing the number of tests makes the threshold
stricter and the analysis weaker on both counts.
\end{keybox}

**And no driving QTN is ever its cluster's representative --- 0 of 21.** That is
not chance. QTN sit in unusually large clusters: the 21 QTN-bearing clusters have
sizes 2, 4, 4, 5, 5, 8, 13, 21, 22, 31, 38, 55, 87, 100, 133, 141, 150, 248, 272,
311 and 354, against a genome-wide **median cluster size of 1** (mean $\approx$ 3).
That is the expected consequence of selection extending LD around a causal site
--- the QTN neighbourhood *is* a large linkage block, which is what $ld_w$
responds to --- but it means the chance that any particular member is the
LD-central one is about $1/\text{size}$, and for clusters of 100--350 that is
negligible.

So testing representatives **never tests a QTN directly**. It relies entirely on
the central marker tagging a variant that may sit at the block's periphery, and
the loss is largest exactly in the big QTN-bearing clusters where it matters
most. This is the mechanism behind the cluster-level power deficits reported
above, which were previously described without one.
...
rows <- list(
 ...
 c("``Fewer tests means a lighter multiplicity correction''","Only if the filter retains signal. Measured: 3x fewer tests gave a BH cutoff 35x MORE severe, because BH is adaptive"),
