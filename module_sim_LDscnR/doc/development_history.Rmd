...
## What the accumulated negatives now show

Four filters have been tested: $ld_w$ top-$k$, MAF, cluster size, and geometric
exclusion. All show strong enrichment for QTN-bearing units --- the geometric
class reaches 30-fold, and $ld_w$ top-5,000 captures 85% of QTN in 2.7% of units
--- and **none converts into precision-recall**.

\begin{keybox}
Enrichment for \emph{containing} a QTN is not the quantity that predicts
detection performance. Filters keep selecting units that hold causal variants
while removing units that were producing the detections.

This supports, by a second route, the conclusion the companion document reaches:
complexity reduction helps because it changes \textbf{what is tested}, and hence
what BH controls, not because it concentrates the search on QTN-rich territory.
Every attempt to add the latter has failed, and this was the cleanest attempt.
\end{keybox}

Two claims made in passing during this work are also withdrawn. Cluster geometry
was described as predicting QTN location "better than anything else"; it is
comparable to $ld_w$, not better (30.4-fold against roughly 31-fold at matched
selectivity). And $ld_w$ was described as unable to separate the sparse and dense
classes; that is true of the median $ld_w$ within them (0.0105 against 0.0099)
but does not make $ld_w$ a poor selector overall, which it is not.
