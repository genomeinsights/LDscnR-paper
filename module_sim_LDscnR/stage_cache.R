...
TAG  <- "nobgs"; CHRS <- 1:10
SRC  <- Sys.getenv("SIM_SRC",   "/Volumes/Nemo/Nemo_sim/regen_sim_data_nobgs")
...
ENVS <- { .e <- Sys.getenv("SIM_ENVS", "")
          if (nzchar(.e)) as.integer(strsplit(.e, ",")[[1]]) else discover_envs(SRC, TAG, V, CC) }
if (!length(ENVS)) stop("no complete env cells in ", SRC)
