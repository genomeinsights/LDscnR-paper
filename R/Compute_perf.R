...
for(file in files){
  ...
  perf_lfmm <- compute_performance(res=res_lfmm_p,
                      map=data$map,
                      GTs=data$GTs)

  perf_emx <- compute_performance(res=res_emx_p,
                                   map=data$map,
                                   GTs=data$GTs)

  out <- list(lfmm=list(res_lfmm,perf_lfmm),emx=list(res_emx,perf_emx),r2_F_ld=r2_F_ld)

  saveRDS(out,paste0(out_folder,basename(file)))

}

eval_grid <- param_grid[
  ,
  .(l_min = l_min_grid),
  by = .(qt, alpha, rho, rho_ld)
]

perf_full <- merge(
  eval_grid,
  perf_lfmm,
  by = c("qt", "alpha", "rho", "rho_ld", "l_min"),
  all.x = TRUE
)
...
perf_full[is.na(precision), precision := 0]
perf_full[is.na(recall), recall := 0]
perf_full[is.na(PR), PR := 0]
...
ggplot(tmp2,aes(E,n_loci)) +
  geom_point() +
  geom_vline(xintercept = median(tmp2$E))

hm_red <- tmp2[E>median(E),
