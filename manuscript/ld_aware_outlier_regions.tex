...
\begin{figure}[t]
  ...
  \fbox{\parbox[c][0.34\linewidth][c]{\linewidth}{\centering\itshape
    Figure placeholder: overlap of the EMMAX regions with the global-specific EcoPeaks
    over the $\tau_C \times \ell_{\min}$ grid---raw overlap, precision, recall,
    rotation-null fold, F1, and region count. The paper operating point
    ($\tau_C = 0.05$, $\ell_{\min} = 3$) marks both the F1 sweet spot and the high-fold
    zone; the strict-corner ``high precision'' is trivial (one or two regions).}}
...
\begin{figure}[t]
  ...
  \fbox{\parbox[c][0.24\linewidth][c]{\linewidth}{\centering\itshape
    Figure placeholder: fold enrichment of global-specific EcoPeak membership by per-SNP
    C-score bin, relative to the genome-wide rate. EMMAX (all $C>0$ bins $24$--$53\times$,
    trend positive) versus LFMM (enrichment declines with $C$).}}
