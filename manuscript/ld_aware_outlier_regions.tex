...
list. Sweeping the same $\tau_C \times \ell_{\min}$ grid used for the structure-null
landscape (Section~\ref{sec:null}) and scoring each cell's overlap with the
global-specific EcoPeaks, both the precision (fraction of regions on an EcoPeak) and
the rotation-null fold enrichment peak in the low-$\tau_C$, low-to-moderate-$\ell_{\min}$
corner, and the operating point used above ($\tau_C = 0.05$, $\ell_{\min} = 3$) sits at
the F1-optimum of precision and recall (Fig.~\ref{fig:sweep}). Recall saturates at
${\sim}0.28$ because most global EcoPeaks are Pacific-derived and absent from the
Atlantic panel, so precision and fold are the informative axes---and on both the chosen
point is near-optimal. The structure-aware null, which uses no truth, and the Kingman
EcoPeaks, an external truth, thus agree on where to operate.
...
\begin{figure}[t]
  ...
  \caption{Calibrating the operating point against an external positive control.}
  \label{fig:sweep}
\end{figure}
