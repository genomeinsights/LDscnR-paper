...
structure is simple and well captured by a handful of latent factors, LFMM is at least
as powerful as EMMAX---a marginally higher region-detection PR-AUC---and its heavier
score tail is genuine power. On the real stickleback genome the ranking reverses:
...
forward-in-time individual-based simulations---where truth is known---LFMM's region-detection PR-AUC is
marginally \emph{higher} than EMMAX's, so a genuine sensitivity advantage is expected to
carry over. Third, the genomic inflation of the LFMM scan is elevated but \emph{not} in
