...
per bin, EMMAX SNPs with any $C > 0$ are enriched $24$--$53$-fold over the $1.3\%$
genome-wide rate, with a positive trend in $C$ (Spearman $\rho = 0.21$ over
${\sim}790{,}000$ SNPs; Fig.~\ref{fig:cscore-enrich}). The LFMM C-score behaves
oppositely: enrichment is present but \emph{declines} with $C$ ($22$-fold down to
$4$-fold; $\rho = 0.06$), so its highest-scoring SNPs are the \emph{least} likely to be
genuine---the signature of an inflated, structure-driven baseline. The EMMAX C-score is
thus an externally calibrated measure of a SNP's outlier evidence and LFMM's is not, the
same specificity/sensitivity split seen at the region level.
...
\begin{figure}[t]
  ...
  \caption{The per-SNP C-score predicts Kingman EcoPeak overlap for EMMAX but not LFMM.}
