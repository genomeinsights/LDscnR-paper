...
rather than signal, dominates. We validate the method on forward-in-time individual-based simulations,
where its region-detection precision--recall matches a latent-factor scan (LFMM),
and on a real stickleback genome scored against an independent set of recurrent
marine--freshwater adaptation peaks. On the real genome the ranking inverts: the
latent-factor scan inflates on the panel's continental-scale structure---flagging
${\sim}10\%$ of the genome, with its highest-scoring variants the least
reproducible---while the LD-aware scan, built on an explicit LD-independent kinship,
stays concentrated on independently corroborated loci. Simulated power is thus a best
case that need not transfer to a structure-rich real genome.
\end{abstract}

\section{Introduction}

Methods for detecting selection and genotype--environment association are almost
universally benchmarked on simulations, and a method's standing in the field rests
heavily on that performance; yet the population structure of a simulation is typically
far simpler than that of a real sampled genome, so a method that leads on simulated
benchmarks can be the one that inflates on real data---precisely where its model of the
confounding structure ceases to fit.
...
multiple-testing burden is absorbed \emph{structurally}, in layers, rather than by a
single genome-wide correction. First, the C-score integrates each locus's evidence
across the $(\rho, q^*)$ grid with an internal Benjamini--Hochberg correction
\citep{benjamini1995controlling} applied \emph{among the local-LD candidates only},
so per-marker multiplicity is controlled within the candidate set rather than over
the whole genome. Second, the size filter \lmin\ removes the small clusters that
dominate false positives. Third, the surviving regions are judged against the
structure-preserving null directly: the region count under the null gives the
operating-point false-discovery ratio, and each region additionally receives a
location-matched permutation $p$-value (below). The threshold \tauc\ therefore need
only fall within a broad safe region rather than on a knife-edge.
...
Across a discovered region set these per-region $p$-values are
Benjamini--Hochberg-adjusted to a region-level false-discovery rate $q_R$. This
$q_R$---not a per-marker Bonferroni over the hundreds of thousands of tested
SNPs---is the multiple-testing correction the method \emph{reports}, the per-SNP
burden having already been carried at the C-score and size-filter stages above; the
region set, of order tens rather than $10^5$, is what remains to be corrected. On the
stickleback data (Section~\ref{sec:threenull}) all $17$ regions clear $q_R < 0.05$
against the genetic and both permutation nulls (largest $q_R = 0.025$), and $14$ of
$17$ against the spatial null---these are false-discovery-controlled region calls,
not nominal $p$-values.

\paragraph{A second-tier consistency score over the operating grid.}
The operating point $(\tauc, \lmin)$ is itself a nuisance choice, and selecting the
cell that maximises the number of significant regions would re-introduce, at the grid
level, exactly the multiple-testing problem the region-level FDR controls within a
cell. We resolve it as the C-score resolves the marker-level nuisance parameters---by
integration. Over a grid $\mathcal{G}$ of operating points, let each cell $g$ yield
its significant regions (those with $q_R(g) < \alpha_R$ at a region-level FDR target
$\alpha_R$, here $0.05$), and let
$\mathcal{U} = \{\, g \in \mathcal{G} : g \text{ yields}\ge 1 \text{ significant region}\,\}$
be the \emph{usable} grid---the part of parameter space in which the method finds
anything at all. The second-tier C-score of a genomic locus $\ell$ is the fraction of
usable cells in which it is recovered as a significant region,
\begin{equation}
  C^{(2)}_\ell \;=\; \frac{1}{|\mathcal{U}|} \sum_{g \in \mathcal{U}}
    \mathbb{1}\!\left[\,\exists\ \text{a region } R \ni \ell \ \text{with}\ q_R(g) < \alpha_R \,\right]
    \;\in\; (0, 1].
  \label{eq:cscore2}
\end{equation}
This is Eq.~\ref{eq:cscore} one level up: the marker C-score integrates
candidacy-and-significance over the $(\rho, q^*)$ grid, while $C^{(2)}$ integrates
region significance over the $(\tauc, \lmin)$ grid---two nested grids, the same
consistency principle. It is robust to grid selection by construction: a locus
significant only in the single cell that maximises the count scores
$C^{(2)} = 1/|\mathcal{U}| \approx 0$, so credibility requires significance
\emph{across} operating points rather than at a cherry-picked one. And it grades
regions by parameter-robustness---$C^{(2)} \to 1$ marks a locus significant
throughout the usable space, whereas a small but positive $C^{(2)}$ marks one
significant only in a narrow slice and hence \emph{fragile} to the threshold choice.
Like the marker C-score, $C^{(2)}$ is a consistency ranking, not a hard filter; it
does not abolish parameter dependence but lifts it from the choice of a single cell
to the choice of a grid \emph{range} and a null---a broad, reportable choice rather
than a cherry-pick. We therefore report the ranked loci together with the grid range,
treating low-$C^{(2)}$ regions explicitly as parameter-sensitive.
...
no region at its own locus more than occasionally: every region clears the
Benjamini--Hochberg region-level false-discovery rate $q_R < 0.05$ (largest
$q_R = 0.025$; median location-matched overlap $0.01$), so all $17$ survive
multiple-testing correction, not merely a nominal threshold. A \emph{regional}
refinement---permuting
...
\begin{table}[h]
  ...
  $p$-values are location-matched overlap tests (Section~\ref{sec:null}); the final
  column counts per-region $p < 0.05$. Under Benjamini--Hochberg region-level FDR the
  counts are $17/17$ for the genetic and both permutation nulls (largest
  $q_R = 0.025$) and $14/17$ for the spatial null, so the surviving calls are
  false-discovery-controlled, not nominal. $^{\dagger}$~$p$-floor $1/(1+B)$ at
  $B = 100$ (genetic); the permutation and spatial nulls use $B = 200$ (floor
  $0.005$).}
  \label{tab:threenull}
\end{table}
...
The external control also validates the \emph{operating point}, not just the region
list. We swept the same $\tau_C \times \ell_{\min}$ grid used for the structure-null
landscape (Section~\ref{sec:null}) and scored each cell's overlap with the
global-specific EcoPeaks (Fig.~\ref{fig:sweep}). Two of the four scored quantities
divide by the number of regions in a cell and are therefore not estimable when a cell
holds only a handful: a single region that happens to hit gives a precision of $1$, and a
near-empty rotation null gives an arbitrarily large fold. Cells with fewer than five
regions are masked throughout. This is a severe restriction for EMMAX---only $33$ of the
$270$ cells survive it, the C-score falling below five regions almost everywhere outside
the low-$\tau_C$ column---and a mild one for LFMM, where $247$ cells survive because a
large block of SNPs carries $C$ near $1$ at every threshold.

On the masked grid the precision (fraction of regions on an EcoPeak) does peak in the
low-$\tau_C$ corner, reaching $0.40$ at $\tau_C = 0.05$, $\ell_{\min} = 6$--$9$. The
rotation-null fold does not: it is largest along the $\ell_{\min} = 1$ edge at high
$\tau_C$ ($46\times$ at $\tau_C = 0.70$), in cells sitting at the masking floor, so it
should be read as a consistency check rather than as an optimisation target. The
operating point used above ($\tau_C = 0.05$, $\ell_{\min} = 3$) is not the F1 maximum---
that falls at $\tau_C = 0.10$, $\ell_{\min} = 1$ ($F_1 = 0.197$ against $0.189$)---but it
ranks third of the $33$ estimable cells and carries the higher precision of the two
($0.29$ versus $0.20$). Recall saturates at ${\sim}0.28$ because most global EcoPeaks are
Pacific-derived and absent from the Atlantic panel; since recall is capped by biology
rather than by the threshold, F1 should not be over-weighted here, and precision is the
axis on which the chosen point is close to the best available. The structure-aware null,
which uses no truth, and the Kingman EcoPeaks, an external truth, thus point to the same
corner of the grid, without either singling out one cell.

The C-score is moreover validated as a graded per-SNP statistic, not only at the region
level. Binning every SNP by its C-score---the fraction of $(\rho, q^*)$ cells in which it
was a valid local-LD outlier candidate---and measuring global-specific EcoPeak membership
per bin (Fig.~\ref{fig:cscore-enrich}), the informative signal is the \emph{step} from
$C = 0$ to $C > 0$: EMMAX SNPs with any $C > 0$ are enriched $24$--$53$-fold over the
$1.3\%$ genome-wide rate, while the ${\sim}789{,}000$ SNPs at $C = 0$ sit at the
background rate ($0.9\times$). Within $C > 0$ neither method shows a rising trend---EMMAX
peaks in the $(0.05, 0.1]$ bin and falls to $23.5\times$ in the top bin, and LFMM falls
monotonically from $22\times$ to $3.8\times$. The positive rank correlations
(Spearman $\rho = 0.21$ for EMMAX, $0.06$ for LFMM) are therefore a restatement of that
step and not evidence of a graded trend, since the $C = 0$ class dominates both.

What separates the two engines is the top of the scale. In the highest bin ($C > 0.5$)
EMMAX places only $13$ SNPs and they are still enriched $23.5\times$, whereas LFMM places
$782$ SNPs there and they are enriched just $3.8\times$. LFMM thus awards high C-scores to
sixty times as many SNPs, and those SNPs are barely better than background---the signature
of an inflated, structure-driven baseline. The EMMAX C-score remains externally calibrated
at its extreme and LFMM's does not, the same specificity/sensitivity split seen at the
region level; the caveat is that C should be read as a candidate flag rather than as a
finely graded strength of evidence.

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{kingman_sweep_EMMAX_c155specific}
  \caption{Calibrating the operating point against an external positive control. Cells
  holding fewer than five regions are masked in the precision, fold and F1 panels, where
  those statistics are not estimable; only $33$ of $270$ cells survive for EMMAX. The
  cross marks the operating point used in the text and the circle the F1 maximum---they
  are adjacent but not identical.}
  \label{fig:sweep}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.9\linewidth]{kingman_cscore_enrichment}
  \caption{Per-SNP C-score versus Kingman global-specific EcoPeak membership. The
  informative contrast is $C = 0$ versus $C > 0$; within $C > 0$ the trend is
  flat-to-declining for EMMAX and monotonically declining for LFMM. At the top of the
  scale EMMAX retains $23.5\times$ enrichment on $13$ SNPs while LFMM falls to
  $3.8\times$ on $782$.}
  \label{fig:cscore-enrich}
\end{figure}
...
This structure-model dependence has a sharp empirical consequence that reconciles the
simulated and the real analyses. On the forward-in-time individual-based simulations, where population
structure is simple and well captured by a handful of latent factors, LFMM recovers markedly
more true regions than EMMAX (recall $0.325$ vs $0.231$ across the $27$ operating
points) at essentially equal precision ($0.337$ vs $0.351$), and its heavier score tail
is genuine sensitivity. On the real stickleback genome the ranking reverses:
against the independent Kingman EcoPeaks the EMMAX regions are far more concentrated
(a $19.5\times$ region-level overlap, and a per-SNP C-score whose bins predict EcoPeak
membership monotonically, Spearman $\rho = 0.21$), whereas LFMM casts a wider, noisier
net whose \emph{highest}-scoring SNPs are the \emph{least} likely to be real (enrichment
\emph{declines} with $C$, $\rho = 0.06$). The economical explanation is that the
continental-scale isolation-by-distance and demography of the real panel are not fully
absorbed by five latent factors, so residual structure inflates the LFMM baseline, while
the explicit, $w_j$-pruned kinship corrects it directly. The general lesson is that a
method's simulated power is a \emph{best case}, realised only when its structure model
happens to fit: clean simulated structure flatters the latent-factor engine, and a
structure-rich real genome rewards the explicit-kinship one---so simulation benchmarks
should not be read as fixed method rankings. Two caveats keep this honest. The Kingman
truth is a set of sharp, discrete peaks that matches EMMAX's specific detection profile,
so LFMM's additional, diffuse detections are penalised rather than credited at the region
level; but the SNP-level C-score comparison is internal to the real data, does not depend
on that truth set, and on its own shows the LFMM score to be miscalibrated where the
EMMAX score is not.

\paragraph{Higher sensitivity, not inflation.}
An alternative to the inflation reading fits the evidence better, and the decisive test
now supports it: LFMM detects \emph{more true positives} than EMMAX rather than more
artifacts. First and most directly, a within-locality permutation null---ecotype shuffled
among populations \emph{inside} each of the four sampling regions, which holds
between-region structure fixed and so isolates parallel marine--freshwater signal---is
cleared by \emph{all} $90$ LFMM regions (location-matched region-level $q_R < 0.05$;
permuted surrogates reconstruct the observed loci with median frequency $0$), the Chr1
inversion ($p = 0.03$) and the \emph{Eda} region ($p = 0.01$) included. Second, on the
forward-in-time individual-based simulations---where truth is known---LFMM recovers markedly \emph{more} true
regions than EMMAX (recall $0.325$ vs $0.231$) at essentially equal precision ($0.337$
vs $0.351$), so a genuine sensitivity advantage---not a uniform superiority---is
expected to carry over. Third, the genomic inflation of the LFMM scan is elevated but \emph{not} in
the manner of unmodelled structure: on the raw statistics $\lambda \approx 1.7$, yet it is
essentially \emph{insensitive to the number of latent factors} ($\lambda \approx
1.6$--$1.9$ for $K = 1$ to $12$). Structure the factors fail to capture would be absorbed
as $K$ grows, driving $\lambda$ toward one; a $\lambda$ that does not respond to $K$
instead points to diffuse polygenic signal---as in LD-score regression, where $\lambda >
1$ reflects polygenicity and not only confounding---consistent with the permutation result
above. (An earlier version of this manuscript reported $\lambda \approx 1.0$ for LFMM;
that value was measured \emph{after} genomic control, which rescales the median to one by
construction and therefore cannot diagnose inflation. The uncorrected value is $\approx
1.7$, while EMMAX's kinship-corrected scan on the same data is well calibrated at $\lambda
\approx 1.09$.) Under this reading LFMM's extra regions (${\sim}90$ vs $17$) are further
real marine--freshwater loci that EMMAX's more conservative scan and Kingman's cohort- and
geography-limited EcoPeaks both miss; because Kingman is a \emph{positive} control only, a
region's absence from it is not evidence that it is false. We therefore do not rank the
engines on the real genome: EMMAX is more \emph{specific} to the independently corroborated
architecture, LFMM more \emph{sensitive}. The one observation still cutting the other way
is that LFMM's per-SNP C-score enrichment against the Kingman peaks \emph{declines} at its
high end, which a pure extra-true-positives account does not predict---though
panel-specific or non-recurrent loci would produce the same pattern. The remaining
structure nulls and the paired EMMAX runs are in progress; the within-region null, which
most directly separates adaptation from structure, is already decisive.

\section{Supplementary: calibrating the kinship on the simulations}
\label{sec:supp-grm}

We validate the $\lambda \ge 1$ rule of Section~\ref{sec:grm} on the forward-in-time individual-based
...
the kinship. The two cutoffs behave as the rule predicts: the
complexity-reduction kinship is calibrated ($\lambda = 1.04$), while the
permissive ``below background LD'' cutoff ($\omega \approx b$, retaining $96\%$
of markers) sits below one ($\lambda = 0.93$), i.e.\ deflated.

The choice of kinship basis has little effect on region detection once the
kinship is pruned at all. Across region-size filters $\ell_{\min}$, these two kinships
return region-detection PR-AUC within one standard error of each other at every
$\ell_{\min}$ ($0.220$ vs $0.210$, $0.235$ vs
$0.227$, $0.274$ vs $0.282$, $0.248$ vs $0.250$; SE $\approx 0.06$, V2\_c1), with no consistent ordering---the deflated kinship is
marginally behind at small $\ell_{\min}$ and marginally ahead at large. The same
inertness appears independently on the stickleback panel, where kinships built
from eight different bases spanning more than a twentyfold range in marker count
give comparable discovery counts and permutation significance throughout.
Leaving the kinship unpruned is the only setting that costs anything, and there
the loss depends on the cluster statistic: a consensus-dosage test collapses,
while a Simes test over the same clusters remains significant. The consensus
dosage is itself a block-level summary, so an unpruned kinship that has absorbed
LD-block structure is near-collinear with it; Simes tests individual markers,
which retain variation the block-level kinship has not absorbed.

This is the expected consequence of the mechanism argued in
Section~\ref{sec:grm}: on weak-LD data a permissive $w_j$ cutoff causes
\emph{global deflation} rather than signal loss---the low-LD QTN are harmless in
the kinship---and global deflation is largely absorbed by the region-size filter.
The practical recommendation is therefore to prune the kinship, not to prune it
in one particular way; $\lambda$ remains the useful diagnostic, since it detects
the global deflation that a marker count alone does not.
