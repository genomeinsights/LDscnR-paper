...
kinship correction, inflating the surrogate scans and over-charging the null. The
number of null peaks per surrogate is a direct calibration check: a well-specified
null produces at most a handful, whereas a surrogate that rivals the observed scan
in peak count is inflated and mis-specified (Section~\ref{sec:threenull}).
...
\subsection{Three structure nulls on a stickleback marine--freshwater scan}
\label{sec:threenull}
...
region set fixed, we read it against three surrogate nulls that differ only in
their structure basis (Table~\ref{tab:threenull}); the median number of C-score
peaks per surrogate is the calibration diagnostic that interprets each.
...
The \emph{permutation} null does apportion it. Shuffling the marine/freshwater
label among the $35$ populations preserves population structure while breaking the
ecotype--genotype link; it rebuilds the chromosome-1 inversion at matching strength
in only ${\sim}1.5\%$ of permutations ($p = 0.015$), but rebuilds the
\textit{Eda}/chromosome-4 peaks ${\sim}11\%$ of the time ($p = 0.10$). The
inversion is thus the more ecotype-specific signal, whereas chromosome-4
differentiation is shared broadly enough across populations that label permutation
reproduces it; seven of the $17$ regions reach $p < 0.05$. A pooled genome-wide
count-FDR against the same null is far blunter---dominated by a few permutations
that light up whole structured blocks, it retains essentially one region---so the
location-matched test is what makes this null usable.

The \emph{spatial} basis instead \emph{over}-charges the null. A Gaussian random
field over the sampling coordinates carpets the genome with a median of $792$ peaks
per surrogate---against $1729$ observed---so surrogate regions overlap the observed
loci by sheer density (median overlap $0.59$) and nothing survives, the inversion
included ($p = 0.26$). The diagnosis is mis-specification, not a real spatial
confound: a smooth field carries autocorrelation the genetic kinship does not
remove, so the surrogate scans are globally inflated, and the peak-count diagnostic
($792 \gg 0$) flags this directly. The mismatch is structural---marine--freshwater
divergence is a \emph{parallel} habitat contrast, the same freshwater alleles
recruited at independent, geographically scattered sites, which is exactly the
pattern a smooth field cannot represent. On the simulations, where the environment
genuinely is a smooth gradient, the identical spatial null is well calibrated and
recovers the causal regions (Section~\ref{sec:null}); it fails here only because
the basis does not match the data.

Together the three nulls give a specificity-then-attribution reading: the genetic
null certifies that the regions are real method output, and the permutation
null---matched to the discrete, parallel structure of the contrast---ranks them by
how far each exceeds population structure, placing the inversion ahead of the
broadly shared chromosome-4 differentiation.

\begin{table}[h]
  \centering
  \begin{tabular}{llccc}
    \hline
    null (basis) & controls for & med.\ null & Chr1 inv. & \textit{Eda}/Chr4 \\
                 &              & peaks/surr. & $p$ & $p$ \\
    \hline
    genetic (MVN, $\mathbf{K}$) & method specificity   & $0$   & $0.010^{\dagger}$ & $0.010^{\dagger}$ \\
    permutation (among pops)    & population structure  & $6$   & $0.015$           & $0.105$ \\
    spatial (GP kernel)         & spatial autocorr.\    & $792$ & $0.264$           & $0.702$ \\
    \hline
  \end{tabular}
  \caption{Three structure nulls on the stickleback scan (observed: $1729$
  C-score markers forming $17$ regions). The median number of C-score peaks per
  surrogate is the null-calibration diagnostic: the genetic basis is silent
  (a specificity control on the method), the permutation basis fires only
  occasionally (attribution against population structure), and the spatial basis is
  globally inflated ($792 \gg 0$)---a smooth field mismatched to a parallel habitat
  contrast. Region $p$-values are location-matched overlap tests
  (Section~\ref{sec:null}). $^{\dagger}$~$p$-floor $1/(1+B)$ at $B = 100$; the
  permutation and spatial nulls use $B = 200$.}
  \label{tab:threenull}
\end{table}
