...
## main-analysis colour (Simes) and are drawn DASHED rather than a distinct
## hue. emmax_snp_clustered/lfmm_snp_clustered keep their own pastel colour,
## solid, unchanged.
...
p1 <- ggplot(fp_neutral, aes(cell_label, mean_FP, colour = arm, linetype = arm, group = arm)) +
...
p2 <- ggplot(fp_neutral_size_bottom, aes(size_bin, mean_FP, colour = arm, linetype = arm, group = arm)) +
  geom_line(alpha = 0.5, linewidth = 0.4, position = position_dodge(width = 0.4)) +
  geom_pointrange(aes(ymin = pmax(0, mean_FP - FP_SE), ymax = mean_FP + FP_SE),
                  position = position_dodge(width = 0.4), size = 0.3) +
  facet_wrap(~tag, nrow = 1) +
  scale_colour_manual(values = ARM_COLOURS, name = "method", guide = "none") +
  scale_linetype_manual(values = ARM_LINETYPES, name = "method", guide = "none") +
  scale_y_log10() +
