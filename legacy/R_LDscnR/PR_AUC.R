...
auc_cummax_PR <- function(scored, p_name, fixed_r2_th = NULL, n_perm = 200, seed = NULL) {
  ...
  sub <- if (is.null(fixed_r2_th)) scored else scored[get(.r2_col(scored)) == fixed_r2_th]
...
run_C_score_threshold_grid <- function(marker_C_score, map, el, qtn_ld_table,
  ...
  or_table <- get_ORs_at_C_score_threshold(marker_C_score, el, C_score_grid, r2_th, l_min = l_min, bp_th = bp_th)
...
extract_alpha_baseline <- function(scored, p_name, sel_r2_th, sel_l_min, sel_th_ldw = 0) {
  r2_col <- .r2_col(scored)
  sub <- scored[th_ldw == sel_th_ldw & get(r2_col) == sel_r2_th & l_min == sel_l_min]
  data.table(
    alpha = sub$alpha,
...
#   "./parsed_sim_data/adapt_bgs_chr2_V2_c1_env1.rds",
#   p_cols = p_cols, th_ldw_grid = th_ldw_grid, r2_grid = r2_grid, lmin_grid = lmin_grid,alpha_grid = alpha_grid,r2_grid_scale = "rho",
#   return_intermediates = TRUE
# )
#
#
# marker_C <- compute_marker_C_score(res$scored, p_name = c("EMX"), fixed_r2_th = 0.5)
# map_C <- attach_C_score_to_map(res$map, marker_C)
...
# plot_OR_manhattan(map_C, value_col = "lfmm_F", clusters = final_clusters,
#                   value_label = "C-score", color_by = "C_score")
#
# qtn_C    <- compute_QTN_C_score(res$scored, p_name = "EMX", fixed_r2_th = 0.5, map = res$map)
# auc_res  <- auc_cummax_PR(res$scored, p_name = "EMX", n_perm = 200, seed = 1)
#
#
#
...
# )
# alpha_curve <- extract_alpha_baseline(res$scored, p_name = "LFMM",sel_r2_th = sel_r2_th, sel_l_min = sel_l_min)
#
