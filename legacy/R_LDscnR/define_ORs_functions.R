...
## standalone use if that script isn't sourced. Needed unconditionally now
## that r2_grid_scale defaults to "rho" in run_and_cluster().
...
# LD-decay curve, used to (i) convert r2_grid from the rho scale
# to actual r^2 thresholds for clustering if r2_grid_scale = "rho"
# (the default), and (ii) report which rho the fixed bp_th_cluster
# distance corresponds to (bp_th_cluster_rho), for reference.
#
# IMPORTANT: with the default r2_grid_scale = "rho", the returned
# $outliers$r2_th column holds the RHO values you supplied in
# r2_grid (not raw r^2) -- this is deliberate, since rho is what's
# actually meaningful/comparable across chromosomes with different
# decay rates, and it means fixed_r2_th/sel_r2_th arguments
# elsewhere in the pipeline "just work" on the scale you're
# actually using without any extra bookkeeping. The raw r^2
# value actually used for LD_igraph_components() clustering is
# kept in a secondary column, $outliers$r2_th_linear, for
# reference/lookup if you need it. If you pass r2_grid_scale =
# "r2" instead (or decay_abc = NULL), r2_th holds raw r^2 as
# before and no r2_th_linear column is added (nothing to
## distinguish it from).
#----------------------------------------------------------
run_and_cluster <- function(map, GTs, ld_ws, p_cols, th_ldw_grid, r2_grid, lmin_grid,
  ...
  r2_grid_input  <- r2_grid   ## what the caller actually supplied (rho, or raw r2)
  r2_grid_actual <- r2_grid   ## what run_one_grid()/clustering actually uses (always raw r2)
  ...
  if (!is.null(decay_abc)) {
    ...
    if (identical(r2_grid_scale, "rho")) {
      r2_grid_actual <- ld_from_rho(b = decay_abc$b, c = decay_abc$c, rho = r2_grid_input)
      message("r2_grid (rho -> r2): ", paste(sprintf("%.3f->%.3f", r2_grid_input, r2_grid_actual), collapse = ", "))
    }
  } else if (identical(r2_grid_scale, "rho")) {
    stop("r2_grid_scale = 'rho' (the default) requires decay_abc = list(a=, b=, c=) to convert ",
         "rho -> r2 for clustering. Pass decay_abc, or set r2_grid_scale = 'r2' explicitly if you ",
         "don't have a fitted decay curve and want to work on the raw r^2 scale instead.")
  }
  ...
  outliers <- rbindlist(lapply(seq_len(nrow(param_grid)), function(i) {
    ...
    run_one_grid(map = map, el = el, ld_ws = ld_ws,
                 ...
                 r2_grid = r2_grid_actual, lmin_grid = lmin_grid,
                 bp_th = bp_th_cluster, cores = cores)
  }), fill = TRUE)

  ## outliers$r2_th currently holds r2_grid_actual (raw r^2) values, from
  ## run_one_grid()'s internal loop. r2_th_linear ALWAYS ends up holding
  ## the actual raw r^2 used for clustering -- identical to r2_th when
  ## r2_grid_scale = "r2" (nothing to relabel), or the value r2_th was
  ## relabeled FROM when r2_grid_scale = "rho". Downstream code can
  ## always rely on r2_th_linear being present, on whichever scale.
  if (identical(r2_grid_scale, "rho")) {
    rho_lookup <- setNames(r2_grid_input, as.character(r2_grid_actual))
    outliers[, r2_th_linear := r2_th]
    outliers[, r2_th := rho_lookup[as.character(r2_th_linear)]]
  } else {
    outliers[, r2_th_linear := r2_th]
  }
...
#
# NOTE: `r2_th` holds whatever scale run_and_cluster() was called
# with (rho by default; see its docs) -- so fixed_r2_th here
# always means "on that same scale", no separate lookup needed.
#----------------------------------------------------------
...
#
# r2_th is the LABEL (whatever scale you're working in -- rho or
# raw r^2), used for filtering/matching elsewhere in the pipeline.
# r2_th_linear is the ACTUAL raw r^2 value used for
# LD_igraph_components() clustering -- defaults to r2_th itself,
# which is only correct if r2_th is ALREADY on the raw r^2 scale.
# If you're working on the rho scale, pass the corresponding raw
# r^2 explicitly via r2_th_linear (run_ORs_C_score_sweep() below
# does this automatically by looking it up from `scored`).
#----------------------------------------------------------
get_ORs_at_C_score_threshold <- function(marker_C_score, el, C_score_grid, r2_th,
                                         r2_th_linear = r2_th, l_min = 1, bp_th = Inf) {

  rbindlist(lapply(C_score_grid, function(C_th) {

    called_markers <- marker_C_score[C_score >= C_th, marker]
    row <- data.table(C_score_threshold = C_th, r2_th = r2_th, r2_th_linear = r2_th_linear, l_min = l_min)
    ...
    clusters_dt <- LD_igraph_components(el = el, markers = called_markers, r2_th = r2_th_linear, bp_th = bp_th)
...
#
# r2_th_grid values are matched against `scored$r2_th` (whatever
# scale that's on -- rho by default). For the actual clustering
# step, the corresponding raw r^2 is looked up from
# `scored$r2_th_linear` automatically -- you never need to supply
# both scales yourself, just use r2_th_grid on the same scale as
# the `scored` table you're passing in.
#----------------------------------------------------------
run_ORs_C_score_sweep <- function(scored, el, p_name, r2_th_grid, C_score_grid,
                                  l_min = 1, bp_th = Inf, filter_fun = NULL) {

  has_linear <- "r2_th_linear" %in% names(scored)
  ...
  rbindlist(lapply(r2_th_grid, function(r2_th) {
    ...
    r2_th_linear <- r2_th
    if (has_linear) {
      lin_vals <- unique(scored[r2_th == r2_th, r2_th_linear])
      if (length(lin_vals) == 1) {
        r2_th_linear <- lin_vals
      } else if (length(lin_vals) > 1) {
        stop("Multiple r2_th_linear values found for r2_th = ", r2_th, ". `scored` likely spans ",
             "multiple files/decay curves with different rho->r2 mappings for the same rho value -- ",
             "run this per file (before pooling), or pass a single-file `scored`.")
      }
      ## length(lin_vals) == 0 (r2_th not present at all) falls through to
      ## r2_th_linear <- r2_th, same as if scored had no r2_th_linear column
    }

    get_ORs_at_C_score_threshold(mc, el, C_score_grid, r2_th = r2_th, r2_th_linear = r2_th_linear,
